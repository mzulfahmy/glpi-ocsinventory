...
function getCurrentNumbers() {
	...
	$cur_count = $count;
	$sql = "SELECT id FROM glpi_computers_softwareversions LIMIT 1";
	$query = mysqli_query($glpiconn, $sql);
	$exec = mysqli_fetch_assoc($query);
	return $count + $exec['id'];
}

$sql = "SELECT id,computers_id,softwareversions_id FROM glpi_computers_softwareversions WHERE date_install='0000-00-00' AND is_deleted_computer='0' AND id BETWEEN ".getCurrentNumbers()." AND ".(getCurrentNumbers() + $cur_count)." ";
