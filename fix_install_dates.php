...
$sql = "SELECT id,computers_id,softwareversions_id FROM glpi_computers_softwareversions WHERE date_install='0000-00-00' AND is_deleted_computer='0' LIMIT 5000";
...
while( $row = mysqli_fetch_assoc($q1)) {
        ...
        $ocs_install_date = substr($ocs_softwares[0]['INSTALLDATE'], 0, 10);
        if($ocs_install_date != '') {
                // Update
                // Commented for tester
                $glpi_query = "UPDATE glpi_computers_softwareversions SET date_install = '".$ocs_install_date."' WHERE id = '".$row['id']."'";
                //$exec = mysqli_query($glpiconn, $glpi_query);
                //echo $glpi_query ."<br>";
                echo "Software Name:  ".$software_name." <br> Computer Name: ".$computer_name." <br> ID " . $row['id'] . " <br> Latest date: " . $ocs_install_date . "\n <br> <hr>";
        }
}
